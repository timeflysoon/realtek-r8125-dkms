...
# ================= 🔐 鉴权模块（支持自动更新 refresh_token） =================
def get_access_token(current_refresh_token):
    print(">>> [Auth] 刷新令牌...")
    data = {
        'client_id': CLIENT_ID,
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"!! 令牌刷新失败: {r.status_code} {r.text}")
            return None, None
        
        token_data = r.json()
        access_token = token_data['access_token']
        new_refresh_token = token_data.get('refresh_token')  # Microsoft 会返回新的
...
def save_new_refresh_token(token, new_refresh_token):
    ...
    try:
        ...
        print("✅ [Auth] 新 refresh_token 已保存至 OneDrive")
    except Exception as e:
        print(f"⚠️ 保存新 refresh_token 失败: {e}")

def load_refresh_token_from_onedrive(token):
    headers = {'Authorization': f'Bearer {token}'}
    try:
        r = requests.get(f'{GRAPH_URL}/me/drive/root:{REFRESH_TOKEN_FILE}:/content', headers=headers)
        if r.status_code == 200:
            return r.text.strip()
    except:
        pass
...
def task_read_calendar(token):
    print("\n>>> [Task 1] 读取日历保活")
    try:
        r = requests.get(f'{GRAPH_URL}/me/events?$top=1', headers={'Authorization': f'Bearer {token}'}, timeout=15)
        print("    ✅ 操作成功")
    except: 
        print("    ⚠️ 日历读取异常（可忽略）")
...
def task_upload_large_file(token):
    ...
    try:
        ...
        r_session = requests.post(session_url, headers=headers, json={"item": {"@microsoft.graph.conflictBehavior": "rename"}}, timeout=15)
        upload_url = r_session.json()['uploadUrl']
        
        requests.put(upload_url, data=b'\0' * file_size, 
                    headers={'Content-Length': str(file_size), 'Content-Range': f'bytes 0-{file_size-1}/{file_size}'}, timeout=60)
...
# ================= 🏁 主入口 =================
def main():
    # 优先使用环境变量，失败则尝试从 OneDrive 读取
    current_refresh = REFRESH_TOKEN
    if not current_refresh:
        print("⚠️ 环境变量无 refresh_token，尝试从 OneDrive 读取...")
    
    token = None
    new_refresh = None
    
    # 首次尝试
    token, new_refresh = get_access_token(current_refresh)
    
    if not token and REFRESH_TOKEN:  # 环境变量失效时尝试备份
        print("⚠️ 环境变量 refresh_token 可能失效，尝试 OneDrive 备份...")
        # 此处需要先用旧 token 获取新 token（简化处理，实际可再优化）
        pass
