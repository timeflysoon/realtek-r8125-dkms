...
def get_access_token():
    ...
    try:
        ...
        # 【核心逻辑】如果微软发回了新的刷新令牌，则将其传出至 GitHub Action
...
def try_lock(token, today_str, current_period):
    ...
    try:
        ...
        if r.status_code == 201:
            ...
            print(f"❌ [Lock] 错误: 响应码 {r.status_code}。请确保网盘已手动创建 {LOCK_FOLDER} 文件夹。")
            return False
    except:
        return False
...
def task_send_mail(token, old_log, today_str):
    ...
    try:
        ...
        if requests.post(f'{GRAPH_URL}/me/sendMail', headers=headers, json=mail_data, timeout=20).status_code in [200, 202]:
...
def task_upload_large_file(token):
    print("\n>>> [Task 4] 模拟大文件操作")
    headers = {'Authorization': f'Bearer {token}'}
    file_size = random.randint(1, 3) * 1024 * 1024 # 1-3MB
...
# ================= 🏁 主入口 =================
def main():
    # 1. 刷新并获取 Access Token (内部已含 RT 自动回写逻辑)
